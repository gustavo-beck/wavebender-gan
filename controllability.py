...
def infer(params):
    ...
    # Load WaveGlow
    waveglow = torch.load(params.waveglow_path)['model']
    ...
    with torch.no_grad():
        audio_wave = waveglow.infer(mel_wavebender[0, 0,:,:].unsqueeze(0), sigma=params.sigma)
        if params.denoiser_strength > 0:
            audio_wave = denoiser(audio_wave, params.denoiser_strength)
        audio_wave = audio_wave * MAX_WAV_VALUE
    audio_wave = audio_wave.squeeze()
    audio_wave = audio_wave.cpu().numpy()
    audio_wave = audio_wave.astype('int16')
    ...
    # Predict audio from wavebender + gan
    with torch.no_grad():
        audio = waveglow.infer(mel.unsqueeze(0), sigma=params.sigma)
        if params.denoiser_strength > 0:
            audio = denoiser(audio, params.denoiser_strength)
    ...
    with torch.no_grad():
        audio_waveglow = waveglow.infer(mel_tacotron.unsqueeze(0).to(device), sigma=params.sigma)
        if params.denoiser_strength > 0:
            audio_waveglow = denoiser(audio_waveglow, params.denoiser_strength)
        audio_waveglow = audio_waveglow * MAX_WAV_VALUE
    audio_waveglow = audio_waveglow.squeeze()
    audio_waveglow = audio_waveglow.cpu().numpy()
    audio_waveglow = audio_waveglow.astype('int16')
